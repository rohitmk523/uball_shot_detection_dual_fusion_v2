#!/usr/bin/env python3
"""Re-run L1 + hi-res-on-UND on one or all June games using the auto-detected
paint-corner calibration produced by calibrate_auto.py.

Skips clip extraction (clips exist from baseline run). Writes results to
results_auto/ + results_hires_auto/ and final_auto.json. Prints per-game
accuracy and aggregate.

Usage:
  python pipeline/rerun_june_auto.py --game-id 4692eb2b
  python pipeline/rerun_june_auto.py --all
"""
...
def rerun_one(gid: str) -> dict:
    ...
    auto_calib = ROOT / f"data/client_report/triangulation_test/calibration_june_{gid}_auto.json"
    if not auto_calib.exists():
        raise FileNotFoundError(f"missing auto calibration: {auto_calib.name}; "
                                 f"run calibrate_auto.py first")

    results_auto = G / "results_auto"
    results_hires_auto = G / "results_hires_auto"
    for d in (results_auto, results_hires_auto):
    ...
    env = {**os.environ,
           "CALIB_JUNE_AUTO": "1",
           "CALIB_JUNE_JSON": str(auto_calib)}
    ...
    und_targets = []
    for s in shots:
        rp = results_auto / f"{s['name']}.json"
        if not rp.exists():
            continue
        d = json.loads(rp.read_text())
        if d.get('verdict', '').startswith('UNDECIDED'):
            und_targets.append(s['name'])
    print(f"[hi-res-auto] {len(und_targets)} UND shots")
    if und_targets:
        r = subprocess.run(
            [sys.executable, "pipeline/triangulate_shot.py",
             "--shots-json", str(pipeline_manifest),
             "--clips-dir", str(clips),
             "--out-dir", str(results_hires_auto),
    ...
    for s in shots:
        ...
        rph = results_hires_auto / f"{s['name']}.json"
        if rph.exists():
        ...
        rows.append(dict(name=s['name'], gt=s['gt'], verdict=v,
                         layer=layer, cat=c))
    (G / "final_auto.json").write_text(json.dumps(rows, indent=2))
    return dict(gid=gid, roll=dict(roll), by_class={k: dict(v) for k, v in by_class.items()})
...
def main() -> int:
    ...
    agg_roll = defaultdict(int)
    for gid in target_games:
        print("\n" + "=" * 60)
        result = rerun_one(gid)
        tp, tn, fp, fn, und = print_tally(gid, result['roll'])
        agg_roll['TP'] += tp; agg_roll['TN'] += tn
        agg_roll['FP'] += fp; agg_roll['FN'] += fn
        agg_roll['UND'] += und

    if args.all:
        print("\n" + "=" * 60)
        print_tally("AGGREGATE (4 games)", agg_roll)
    return 0
