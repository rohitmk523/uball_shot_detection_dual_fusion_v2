...
Runs extract_tracks for a list of games SEQUENTIALLY, and records progress:

  * If SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY are set, writes a
    dual_fusion_v2.iterations row (phase=P1_extract, status running->
    done/failed) and updates dual_fusion_v2.games
    (tracks_extracted_at / tracks_s3_key) via the PostgREST API.
  * Tracking is best-effort: a missing service key, a missing
    dual_fusion_v2 schema, or any PostgREST error must NOT fail the
    extraction job. In that case progress is written to S3 instead.

Usage:
  python run_batch.py --split train|val|test|all
  python run_batch.py --games <id1> <id2> ...
"""
...
from extract_tracks import extract_game
...
def _pgrest(method: str, path: str, body: Optional[dict] = None,
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            # Target the dual_fusion_v2 schema (PostgREST exposed schema).
...
def _iteration_create(run_id: str, games: List[str], worst_case_usd: float):
...
def _game_mark_extracted(game_id: str, tracks_key: str):
    return _safe(lambda: _pgrest(
        "PATCH", f"games?id=eq.{game_id}",
        {
            "tracks_extracted_at": datetime.now(timezone.utc).isoformat(),
            "tracks_s3_key": tracks_key,
        },
        prefer="return=minimal",
    ), "game_mark_extracted")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    ap = argparse.ArgumentParser(description="P1 batch track extraction")
    ...
    ap.add_argument("--run-id",
                    ...
                    or datetime.now(timezone.utc).strftime("p1-%Y%m%dT%H%M%SZ"))
    ...
    if tracking:
        _iteration_create(run_id, [g.game_id for g in games], 0.0)
    ...
    for i, g in enumerate(games, 1):
        ...
        try:
            res = extract_game(g, force=args.force)
            rec.update(status=res["status"])
            if res.get("tracks_s3_key"):
                artifacts[g.game_id] = res["tracks_s3_key"]
                rec["tracks_s3_key"] = res["tracks_s3_key"]
                if tracking and res["status"] == "done":
                    _game_mark_extracted(g.game_id, res["tracks_s3_key"])
        except Exception as e:
            overall = "failed"
    ...
    elapsed_hr = (datetime.now(timezone.utc) - started).total_seconds() / 3600.0
