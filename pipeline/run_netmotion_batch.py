#!/usr/bin/env python3
"""
P1b net-motion batch driver.

Runs extract_netmotion for a list of games SEQUENTIALLY and records
progress, mirroring run_batch.py exactly:

  * If SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY are set, writes a
    dual_fusion_v2.iterations row (phase=P1b_netmotion, status running->
    done/failed) and updates dual_fusion_v2.games
    (netmotion_extracted_at / netmotion_s3_key) via the PostgREST API.
...
Idempotent: a game whose netmotion_meta.json already exists is skipped
(unless --force).

Usage:
  python run_netmotion_batch.py --split train|val|test|all
  python run_netmotion_batch.py --games <id1> <id2> ...
"""
...
from extract_netmotion import extract_game
...
def _game_mark_extracted(game_id: str, netmotion_key: str):
    return _safe(lambda: _pgrest(
        "PATCH", f"games?id=eq.{game_id}",
        {
            "netmotion_extracted_at": datetime.now(
                timezone.utc).isoformat(),
            "netmotion_s3_key": netmotion_key,
        },
        prefer="return=minimal",
    ), "game_mark_extracted")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    ap = argparse.ArgumentParser(
        description="P1b batch net-motion extraction"
    )
    ...
    ap.add_argument(
        ...
        or datetime.now(timezone.utc).strftime("p1b-%Y%m%dT%H%M%SZ"),
    )
    ...
    for i, g in enumerate(games, 1):
        ...
        try:
            ...
            if res.get("netmotion_s3_key"):
                artifacts[g.game_id] = res["netmotion_s3_key"]
                rec["netmotion_s3_key"] = res["netmotion_s3_key"]
                if tracking and res["status"] == "done":
                    _game_mark_extracted(
                        g.game_id, res["netmotion_s3_key"]
                    )
        except Exception as e:
            overall = "failed"
