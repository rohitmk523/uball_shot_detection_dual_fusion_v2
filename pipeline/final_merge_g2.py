#!/usr/bin/env python3
"""Game-2 tiered final merge: L1 triangulation -> L2 UND-rerun -> L3 ensemble
-> L5 multi-shot override. (No L4 hi-res yet; game-2 doesn't have a hi-res
results directory.)
"""
...
MAIN = G2 / "results"
RERUN = G2 / "results_und_conf10"
ENS_PATH = G2 / "ensemble_results.json"
MULTI_PATH = G2 / "multi_shot_results.json"
...
def main() -> int:
    ...
    # L3 per-camera ensemble — strong-only fill of UND, plus dual-strong flips
    if ENS_PATH.exists():
        ...
        for r in ens_rows:
            ...
            old = final.get(name, "")
            if ev != old:
                # L3 already applied ensemble_vote rules; only adopt when
                # strong (the script already keeps weak silent)
