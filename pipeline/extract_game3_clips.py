#!/usr/bin/env python3
"""Extract per-shot clips for game-3 (3398befc) with the +10.5 frame NR offset
baked in. NR_SHIFT_S = 10.5 / 29.97 seconds.

Mirror of extract_game2_clips.py. Sync points provided by user:
  FR:405  = NR:415   (offset +10 frames)
  FR:1111 = NR:1122  (offset +11 frames)
Midpoint of +10.5 frames minimises max sync error across the two anchors.
"""
...
CLIPS = G3 / "clips"
FR_FULL = G3 / "3398befc_FR_full.mp4"
NR_FULL = G3 / "3398befc_NR_full.mp4"

PAD_BEFORE = 1.0
PAD_AFTER = 2.5
NR_SHIFT_S = 10.5 / 29.97   # +10.5 frame offset (midpoint of user sync anchors)
...
def main():
    ...
    manifest = json.loads((G3 / "shots_all.json").read_text())
    print(f"[setup] {len(manifest)} shots to extract")
    print(f"[setup] NR offset: +{NR_SHIFT_S*1000:.0f} ms (+10.5 frames)")
    ...
    with ProcessPoolExecutor(max_workers=6) as ex:
        ...
        for f in as_completed(futs):
            ...
            fr_ok = "OK" if (r['fr'].get('ok') or r['fr'].get('skipped')) else "FAIL"
            nr_ok = "OK" if (r['nr'].get('ok') or r['nr'].get('skipped')) else "FAIL"
            print(f"  [{done:>3}/{len(manifest)}] {r['name']:18s}  "
