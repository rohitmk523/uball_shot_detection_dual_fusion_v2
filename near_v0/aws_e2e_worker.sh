...
for row in "${GAMES[@]}"; do
  ...
  python3 near_v0/test_end_to_end.py --game "$G" --video /work/vid.mp4 \
    --manifest "/work/frozen_manifests/$G.json" --t0 "$T0" --t1 "$T1" --stride 3 \
  ...
  aws s3 cp "/work/data/near_detector/e2e_$G.json" "$P/out/e2e_$G.json" >/dev/null 2>&1 || true
  aws s3 cp "/work/out_$G.txt" "$P/out/out_$G.txt" >/dev/null 2>&1 || true
  rm -f /work/vid.mp4
done
